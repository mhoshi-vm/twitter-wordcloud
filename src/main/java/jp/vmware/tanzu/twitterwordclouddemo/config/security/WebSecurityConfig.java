...
public class WebSecurityConfig {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http
				// Add an authentication check filter to enable auto-redirects if the user
				...
				.authorizeRequests(authorizeRequests -> authorizeRequests
						...
						.antMatchers("/", "/webjars/**", "/styles/**", "/images/**").permitAll()
						// Require authentication for all others paths.
						.anyRequest().authenticated())
				// After a successful logout, redirect to /home.
				.logout().logoutSuccessHandler(oidcLogoutSuccessHandler()).logoutSuccessUrl("/home").and()
